...
import { getPublicStatusPage } from "@/modules/statuspages/statuspages.api";
	...
	if (status === "down") {
		return (
			<div className="flex items-center text-red-500 font-medium text-sm">
				<XCircle className="w-4 h-4 mr-1.5" /> Outage
			</div>
		);
	}
	if (status === "maintenance") {
		return (
			<div className="flex items-center text-amber-500 font-medium text-sm">
				<Activity className="w-4 h-4 mr-1.5" /> Maintenance
			</div>
		);
	}
	return (
		<div className="flex items-center text-muted-foreground font-medium text-sm">
			<AlertTriangle className="w-4 h-4 mr-1.5" /> Unknown
		</div>
	);
}
...
function PublicStatusPage() {
	...
	const isMaintenance = page.monitors.some(
		(m: any) => m.monitor.status === "maintenance",
	);
		...
		<div className="min-h-screen bg-neutral-50 dark:bg-neutral-950 font-sans text-neutral-900 dark:text-neutral-50 flex flex-col">
			...
			<div className="w-full bg-white dark:bg-neutral-900 border-b">
				<div className="max-w-3xl mx-auto px-6 py-8 md:py-12">
					...
					<div
						className={cn(
							"rounded-xl p-6 md:p-8 flex items-center shadow-sm text-white transition-colors",
							statusColor,
						)}
					>
						{statusIcon}
					...
					<div className="space-y-6">
						...
						<div className="border-l-2 border-neutral-200 dark:border-neutral-800 pl-4 py-1">
							...
							<p className="text-sm text-muted-foreground mt-1">
								We observed increased latency on our API endpoints. The issue
								has been resolved.
							</p>
						</div>
						{/* Mock Incident 2 */}
			...
			<footer className="w-full py-8 text-center text-sm text-muted-foreground border-t mt-auto">
				...
				<a
					href="/"
					className="font-semibold hover:underline text-neutral-900 dark:text-neutral-100"
				>
					BoringStatus
				</a>
			</footer>

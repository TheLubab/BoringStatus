...
export const getMonitorsForDevtools = createServerFn({ method: "GET" }).handler(
	async (): Promise<
		Pick<DashboardMonitor, "id" | "name" | "status" | "target" | "type">[]
	> => {
...
// Delete ALL monitors
export const deleteAllMonitors = createServerFn({ method: "POST" }).handler(
	async () => {
		const orgId = await requireAuth();
		requireDev();
...
export const triggerStatus = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }) => {
		...
		if (!mon || mon.organizationId !== orgId)
		...
		const metrics = generateMetrics(
			mon.type as MonitorType,
			status === "up" ? 1 : status === "degraded" ? 5 : 10,
			status,
		);
...
export const clearHeartbeats = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }) => {
		...
		await db.transaction(async (tx) => {
			...
			await tx
				.update(monitor)
				.set({ status: "pending", lastCheckAt: null, nextCheckAt: null })
				.where(eq(monitor.id, data.monitorId));
		});
...
const patternGenerators: Record<
	Pattern,
	(pos: number) => { status: HeartbeatStatus; latencyMult: number }
> = {
	stable: () => {
	...
	incident: (pos) => {
		if (pos < 0.33)
			return { status: "up", latencyMult: 0.8 + Math.random() * 0.3 };
		if (pos < 0.66) {
			const p = (pos - 0.33) / 0.33;
			if (p < 0.1 || p > 0.9) return { status: "degraded", latencyMult: 2.5 };
			return {
				status: Math.random() > 0.1 ? "down" : "error",
				latencyMult: 10,
			};
		}
		...
		if (p < 0.3)
			return {
				status: Math.random() > 0.3 ? "up" : "degraded",
				latencyMult: 1.5,
			};
	...
	recovery: (pos) => {
		...
		if (pos < 0.4)
			return {
				status: Math.random() > 0.5 ? "down" : "degraded",
				latencyMult: 5,
			};
		if (pos < 0.6)
			return {
				status: Math.random() > 0.3 ? "degraded" : "up",
				latencyMult: 2,
			};
		if (pos < 0.8)
			return {
...
const generateMetrics = (
	...
	if (type === "http") {
		return {
			...
			contentLength:
				status === "up" ? 12000 + Math.round(Math.random() * 5000) : 0,
		};
	}
	if (type === "ping") {
		return {
	...
	return {
		connect: latency,
		success: status === "up" || status === "degraded",
	};
};
...
export const refreshAggregates = createServerFn({ method: "POST" }).handler(
	async () => {
		await requireAuth();
		requireDev();

...
const TIME_RANGES: Record<
	TimeRange,
	{ label: string; minutes: number; interval: number }
> = {
...
function DevPanel() {
	...
	const simulate = useMutation({
		mutationFn: async (monitorId: string) => {
			const mod = await import("./devtools.server");
			return mod.simulateRealisticHeartbeats({
				data: { monitorId, pattern, ...TIME_RANGES[time] },
			});
		},
	...
	const pending =
		simulate.isPending ||
		clearHeartbeats.isPending ||
		deleteMonitor.isPending ||
		createMonitor.isPending ||
		refreshAgg.isPending ||
		seedDemo.isPending ||
		triggerStatus.isPending ||
		deleteAll.isPending;
	const selectedName = monitor
		? monitors.find((m) => m.id === monitor)?.name
		: null;
			...
			<div style={styles.content}>
				<Section title="Create Monitor">
					<div style={styles.row}>
						<Btn
							onClick={() => createMonitor.mutate("http")}
							disabled={pending}
						>
							+ HTTP
						</Btn>
						<Btn onClick={() => createMonitor.mutate("tcp")} disabled={pending}>
							+ TCP
						</Btn>
						<Btn
							onClick={() => createMonitor.mutate("ping")}
							disabled={pending}
						>
							+ Ping
						</Btn>
						<Btn
							onClick={() => refreshAgg.mutate()}
							disabled={pending}
							color="#666"
						>
							Refresh Agg
						</Btn>
						<Btn onClick={handleSeed} disabled={pending} color="#8b5cf6">
							Seed Demo
						</Btn>
					</div>
				</Section>
				...
				<Section title="Trigger Status">
					<div style={styles.row}>
						<Btn
							onClick={() => triggerStatus.mutate("up")}
							disabled={pending || !monitor}
							color="#22c55e"
						>
							UP
						</Btn>
						<Btn
							onClick={() => triggerStatus.mutate("degraded")}
							disabled={pending || !monitor}
							color="#eab308"
						>
							DEGRADED
						...
						<Btn
							onClick={() => triggerStatus.mutate("down")}
							disabled={pending || !monitor}
							color="#ef4444"
						>
							DOWN
						...
						<Btn
							onClick={() => triggerStatus.mutate("error")}
							disabled={pending || !monitor}
							color="#71717a"
						>
							ERROR
					...
					<div style={styles.row}>
						{Object.entries(TIME_RANGES).map(([k, v]) => (
							<Pill
								key={k}
								active={time === k}
								onClick={() => setTime(k as TimeRange)}
							>
								{v.label}
							</Pill>
						))}
					...
					<div style={{ ...styles.row, marginTop: 6 }}>
						{Object.entries(PATTERNS).map(([k, v]) => (
							<Pill
								key={k}
								active={pattern === k}
								onClick={() => setPattern(k as Pattern)}
							>
								{v}
							</Pill>
						))}
				...
				<Section title="Destructive">
					<div style={styles.row}>
						...
						<Btn
							...
							disabled={pending || !monitors.length}
							style={{ opacity: 0.8 }}
						>
							Delete ALL
						</Btn>
			...
			<div style={styles.log}>
				{log.length
					? log.map((l) => (
							<div
								key={l}
								style={{ color: l.startsWith("✓") ? "#22c55e" : "#888" }}
							>
								{l}
							</div>
						))
